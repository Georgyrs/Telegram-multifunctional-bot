...
def work(message):
    ...
    job_scenarios = {
        'Продавец': [
            {
                'description': '🛒 У вас день больших распродаж. Как будете привлекать покупателей?',
                'options': ['Организовать рекламную акцию', 'Снизить цены', 'Не предпринимать ничего'],
                'outcomes': [
                    ('Рекламная акция привлекла много клиентов, вы заработали $1000!', 1000),
                    ('Снижение цен увеличило продажи, но прибыль уменьшилась. Вы заработали $500.', 500),
                    ('Вы ничего не сделали, и продаж было мало. Потеря $200!', -200)
                ]
            },
            {
                'description': '💳 Один из клиентов жалуется на испорченный товар. Что будете делать?',
                'options': ['Возместить ущерб', 'Предложить скидку', 'Отказать в возврате'],
                'outcomes': [
                    ('Вы компенсировали ущерб, и клиент ушел довольным. Потеря $300.', -300),
                    ('Скидка снизила убытки, и клиент все равно купил товар. Потеря $100.', -100),
                    ('Вы отказали, и клиент ушел, но репутация пострадала. Потеря $50.', -50)
                ]
            }
        ],
        'Дворник': [
            {
                'description': '🧹 Ваша задача - убрать парк после праздника. Как будете действовать?',
                'options': ['Работать быстрее', 'Нанять помощника', 'Не торопиться'],
                'outcomes': [
                    ('Вы быстро убрали парк и заработали $400!', 400),
                    ('Помощник облегчил работу, но вы разделили прибыль. Вы заработали $200.', 200),
                    ('Вы не спешили, и работа затянулась. Потеря $100!', -100)
                ]
            },
            {
                'description': '🚮 В мусорном контейнере оказался драгоценный предмет. Что сделаете?',
                'options': ['Оставить себе', 'Отдать владельцу', 'Игнорировать находку'],
                'outcomes': [
                    ('Вы оставили находку себе и продали ее за $500!', 500),
                    ('Вы вернули вещь владельцу, и он наградил вас $300.', 300),
                    ('Вы проигнорировали находку. Никаких изменений.', 0)
                ]
            }
        ],
        'Программист': [
            {
                'description': '💻 У вас срочный проект. Как поступите?',
                'options': ['Работать всю ночь', 'Нанять фрилансеров', 'Отложить проект'],
                'outcomes': [
                    ('Вы поработали всю ночь и успели. Прибыль $1200!', 1200),
                    ('Фрилансеры помогли закончить вовремя, но это обошлось в $500.', -500),
                    ('Вы отложили проект и потеряли клиента. Потеря $700!', -700)
                ]
            },
            {
                'description': '📱 Ваша программа оказалась популярной. Что сделаете?',
                'options': ['Выпустить обновление', 'Оставить все как есть', 'Пойти в отпуск'],
                'outcomes': [
                    ('Обновление улучшило программу, и вы заработали $800!', 800),
                    ('Никаких изменений. Доход остался на прежнем уровне.', 0),
                    ('Вы пошли в отпуск и потеряли часть прибыли. Потеря $300.', -300)
                ]
            }
        ],
        'Бизнесмен': [
            {
                'description': '💼 Вы получили выгодное предложение. Как поступите?',
                'options': ['Заключить сделку', 'Отказаться', 'Попробовать договориться о лучших условиях'],
                'outcomes': [
                    ('Сделка оказалась успешной! Прибыль $2000!', 2000),
                    ('Вы отказались, и ничего не изменилось.', 0),
                    ('Вы договорились о лучших условиях и заработали $1500.', 1500)
                ]
            },
            {
                'description': '📊 Ваша компания переживает кризис. Что будете делать?',
                'options': ['Инвестировать в маркетинг', 'Сократить расходы', 'Ничего не предпринимать'],
                'outcomes': [
                    ('Маркетинг помог, и вы заработали $1000!', 1000),
                    ('Сокращение расходов помогло сохранить прибыль. Вы заработали $500.', 500),
                    ('Ничего не изменилось, и вы потеряли $800!', -800)
                ]
            }
        ],
        'Проститутка': [
            {
                'description': '💃 Вы встретили клиента с высокими требованиями. Как поступите?',
                'options': ['Стараться угодить', 'Сбежать', 'Попросить больше денег'],
                'outcomes': [
                    ('Клиент был доволен, и вы заработали $1500!', 1500),
                    ('Вы сбежали, и потеряли $300.', -300),
                    ('Клиент заплатил больше, но был недоволен. Потеря $200.', -200)
                ]
            },
            {
                'description': '💋 Вы попали в неприятную ситуацию. Что сделаете?',
                'options': ['Позвонить в полицию', 'Скрыться', 'Попытаться договориться'],
                'outcomes': [
                    ('Полиция помогла вам избежать проблем. Вы избежали убытков.', 0),
                    ('Вы скрылись, но потеряли часть заработка. Потеря $400.', -400),
                    ('Вы договорились и получили $100.', 100)
                ]
            }
        ]
    }

    if job_name in job_scenarios:
        scenarios = job_scenarios[job_name]
    else:
        scenarios = random.choice(list(job_scenarios.values()))

    scenario = random.choice(scenarios)

    random.shuffle(scenario['outcomes'])
    balance = get_balance(user_id, chat_id)
    keyboard = InlineKeyboardMarkup()
    for idx, option in enumerate(scenario['options']):
        keyboard.add(InlineKeyboardButton(text=option, callback_data=f"work_{idx}_{user_id}_{payment}"))
...
def handle_choice(call):
    _, choice_str, original_user_id, payment_str = call.data.split('_')
    user_id = call.from_user.id
    chat_id = call.message.chat.id

    if str(user_id) != original_user_id:
        bot.answer_callback_query(call.id, "Вы не можете принять это решение!")
        return
    ...
    job_name, payment = do_job(user_id, chat_id)
    scenario = random.choice(job_scenarios[job_name])
    outcome_message, outcome_payment = scenario['outcomes'][choice]

    bot.answer_callback_query(call.id, outcome_message)
    balance = get_balance(user_id, chat_id)
    if balance >= 50000:
        ...
        bot.send_message(chat_id, f'<b>Налог 2% уплачен!</b> 💸\n<i>Ваш новый баланс:</i> <b>{int(balance)}$</b>',
                         parse_mode='html')
        bot.send_message(chat_id, f"{outcome_message}\n💰 Ваш итоговый заработок: ${int(payment + outcome_payment)}.")
    else:
        bot.send_message(chat_id, f"{outcome_message}\n💰 Ваш итоговый заработок: ${int(payment + outcome_payment)}.")

    update_balance(user_id, chat_id, outcome_payment)
...
def remove_vip_from_all_users():
    cursor.execute('DELETE FROM user_upgrades WHERE upgrade_name = "VIP"')
    conn.commit()
...
def init_upgrades():
    ...
    cursor.execute('UPDATE upgrades SET cost = 50000 WHERE upgrade_name = "VIP"')

    for upgrade in upgrades:
        cursor.execute('INSERT OR IGNORE INTO upgrades (upgrade_name, cost) VALUES (?, ?)', upgrade)
...
def government_addmoney(message):
    ...
    conn.commit()
    update_balance(1548224823, chat_id, -amount)
